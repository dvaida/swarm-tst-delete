...
// Config holds all configuration settings for swarm-indexer
type Config struct {
	TypesenseURL        string
...
func Load() (*Config, error) {
	cfg := &Config{
		...
		SkipFiles:           parseSkipFiles(getEnvOrDefault("SWARM_INDEXER_SKIP_FILES", ".env,.setenv,*.pem,*.key,credentials.*")),
	}
...
func parseSkipFiles(value string) []string {
	if value == "" {
		return nil
	}
